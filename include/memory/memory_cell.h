//
// Created by xiehao on 2024/12/2.
//
...
private:
     ...
     * 是否是中转对象
     *
     * 在GC标记阶段会将原对象放入Chunk的transfer_table中(因为之前malloc返回的指针只想的是这个对象, 这个对象释放了旧指针就失效了)
     * 然后生成一个克隆对象爱嗯放入Chunk的unused_table中用于后续的内存释放与整理(如果是多线程，GC阶段需要STW,否则内存的数据会被其他线程覆盖掉)
     * 这个属性就是标识是否是这个克隆对象
     */
    bool m_transfer_object;

private:
    // 这里实际类型为MemoryChunk，因为C++不允许互为成员，所以用pvoid代替
